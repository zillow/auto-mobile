...
import { ActionableError, ActiveWindowInfo, ObserveResult, ViewHierarchyResult } from "../../models";
...
export class BaseVisualChange {
  ...
  async observedInteraction(
    ...
    let previousObserveResult: ObserveResult | null = options.previousObserveResult?.viewHierarchy ?? null;
    ...
    const result = await this.takeObservation(blockResult, previousObserveResult, {
   ...
   * @param options - Options controlling observation behavior
   */
  async observedInteractionWithChange(
