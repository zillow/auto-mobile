...
/**
 * Fake Net Server for testing without real sockets or file system.
 */
export class FakeNetServer extends EventEmitter {
  public listening = false;
  private _socketPath: string | null = null;
  private _connectedClients: FakeSocket[] = [];
   ...
   * Returns the server-side socket that can be used to interact with the "client".
   */
  simulateConnection(): FakeSocket {
    if (!this.listening) {
      throw new Error("Server is not listening");
    }
    const serverSocket = new FakeSocket();
    this._connectedClients.push(serverSocket);
    this.emit("connection", serverSocket);
    return serverSocket;
  }

  /**
   * Create a connected pair of sockets (client and server side).
   * Data written to one will be received by the other.
   */
  createConnectedPair(): { clientSocket: FakeSocket; serverSocket: FakeSocket } {
    const serverSocket = this.simulateConnection();
    const clientSocket = new FakeSocket();
    clientSocket.connectToPeer(serverSocket);
  ...
  getClientCount(): number {
    return this._connectedClients.length;
  }
...
/**
 * Factory function to create a FakeNetServer.
 * Can be used to replace `createServer` from node:net in tests.
 */
export function createFakeServer(connectionHandler?: (socket: FakeSocket) => void): FakeNetServer {
