...
import type { Sha256Source, XCTestServiceBundleDownloader } from "../../src/utils/XCTestServiceBundleDownloader";

export class FakeXCTestServiceBundleDownloader implements XCTestServiceBundleDownloader {
  ...
  public async extractBundle(_bundlePath: string, destination: string): Promise<void> {
    ...
    const xctestrunFile = path.join(extractionRoot, "Build", "Products", "XCTestServiceApp_iphonesimulator.xctestrun");
    await fs.writeFile(xctestrunFile, "fake xctestrun");
    await fs.mkdir(path.join(productsDir, "XCTestServiceApp.app"), { recursive: true });
    await fs.mkdir(path.join(productsDir, "XCTestServiceUITests-Runner.app"), { recursive: true });
    await fs.mkdir(path.join(productsDir, "XCTestServiceTests.xctest"), { recursive: true });
  }
}
