import { XCTestServiceManager, XCTestServiceSetupResult } from "../../src/utils/XCTestServiceManager";
import { PerformanceTracker } from "../../src/utils/PerformanceTracker";

/**
 * Fake implementation of XCTestServiceManager for testing
 * Allows configuring service state and asserting operations
 */
export class FakeXCTestServiceManager implements XCTestServiceManager {
  ...
  /**
   * Set whether the XCTestService is installed
   */
  ...
  /**
   * Set whether the XCTestService is running
   */
  ...
  /**
   * Set whether the XCTestService is available (installed AND running)
   */
  ...
  // MARK: - XCTestServiceManager Implementation
  ...
  async start(): Promise<void> {
    ...
    if (this.shouldStartFail) {
      throw new Error("Failed to start XCTestService");
    }
  ...
  async stop(): Promise<void> {
    ...
    if (this.shouldStopFail) {
      throw new Error("Failed to stop XCTestService");
    }

    this.runningState = false;
  }

  async setup(force: boolean = false, perf?: PerformanceTracker): Promise<XCTestServiceSetupResult> {
