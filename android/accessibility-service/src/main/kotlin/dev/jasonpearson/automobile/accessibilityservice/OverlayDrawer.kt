...
class OverlayDrawer(
  ...
  private fun buildRenderState(shape: HighlightShape): HighlightRenderResult {
    ...
    val hasStroke = style == null || style.strokeColor != null || style.strokeWidth != null
    val fillColor = style?.fillColor
    val dashPattern = style?.dashPattern
    ...
    val strokePaint =
        if (hasStroke) {
          val strokeWidth = style?.strokeWidth ?: DEFAULT_STROKE_WIDTH
          if (strokeWidth <= 0f) {
            return HighlightRenderResult(error = "strokeWidth must be greater than 0")
          }

          val strokeColor = style?.strokeColor ?: DEFAULT_STROKE_COLOR
          val parsedStrokeColor = parseColor(strokeColor)
          ...
          if (dashPattern != null) {
            if (dashPattern.isEmpty() || dashPattern.any { it <= 0f }) {
              return HighlightRenderResult(error = "dashPattern must contain positive values")
            }
    ...
    val fillPaint =
        if (fillColor != null) {
          val parsedFillColor = parseColor(fillColor)
  ...
  private fun snapshotHighlights(): List<HighlightEntry> {
    return synchronized(lock) {
      highlights.map { (id, renderState) -> HighlightEntry(id = id, shape = renderState.shape) }
    }
  }
  ...
  private data class HighlightRectResult(
      val rect: RectF? = null,
      val error: String? = null,
  )
