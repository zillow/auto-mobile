 ...
 * ```
 */
class PerfProvider private constructor(
    ...
    companion object {
        ...
        @Volatile
        private var INSTANCE: PerfProvider? = null

        val instance: PerfProvider
            get() = INSTANCE ?: synchronized(this) {
                INSTANCE ?: PerfProvider().also { INSTANCE = it }
            }
     ...
     * Use this for operations that may run concurrently and should be tracked
     * as parallel/sibling entries rather than nested within each other.
     *
     * Any open blocks are closed and moved to completedEntries, preserving their
     * timing data for inclusion in the next flush().
     */
    ...
    /**
     * Track an operation with automatic start/end timing.
    ...
    /**
     * Track a suspend operation with automatic start/end timing.
     * Returns the result of the block.
     */
     ...
     * Returns the timing data as a JsonElement for inclusion in WebSocket messages.
     */
    fun flush(): JsonElement? {
        ...
        val debounceInfo = if (debounceCount > 0) {
            val info = PerfTiming(
                name = "debounce",
                durationMs = 0,
                children = listOf(
     ...
     */
    fun hasData(): Boolean {
        return completedEntries.isNotEmpty() ||
