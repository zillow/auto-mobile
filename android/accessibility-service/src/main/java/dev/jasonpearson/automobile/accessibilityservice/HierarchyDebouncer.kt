...
sealed class HierarchyResult {
     ...
     * Bounds may have changed but text/content is the same.
     */
 ...
 *
 * Uses structural hash comparison to detect when content actually changes
 * vs. when only bounds are changing during animations.
 *
 * Key optimization: During scroll/fling animations, many TYPE_WINDOW_CONTENT_CHANGED
 * events fire but only bounds change. By comparing structural hashes, we can:
 ...
 * @param animationSkipWindowMs How long to skip extractions after detecting animation (default 100ms)
...
class HierarchyDebouncer(
    ...
    @Volatile
    private var suppressFlowEmissions: Boolean = false
    ...
    private val _hierarchyFlow = MutableSharedFlow<HierarchyResult>(
        replay = 1,
    ...
    @Volatile
    private var lastHierarchy: ViewHierarchy? = null

    /**
     * Called when an accessibility event is received that might change the hierarchy.
     * Handles debouncing and decides whether to extract based on animation detection.
     */
     ...
     * This is async - returns immediately after launching the extraction.
     */
    fun extractNow() {
        debounceJob?.cancel()
        inAnimationMode = false
     ...
     * This ensures the hierarchy is extracted and pushed before returning.
     * Use this when you need to guarantee the hierarchy is fresh before sending a result.
     *
     * @param skipFlowEmit If true, skips emitting to the flow. Use this when the caller
     *                     will broadcast the hierarchy directly to avoid race conditions
     *                     with flow-based async broadcasts.
    ...
    fun extractNowBlocking(skipFlowEmit: Boolean = false): ViewHierarchy? {
        ...
        kotlinx.coroutines.runBlocking {
            extractAndCompare(skipFlowEmit)
        }
        return lastHierarchy
    }

    /**
     * Wait for accessibility events to stop firing (quiescence), then extract hierarchy.
     * This is the preferred method for operations that modify UI state (like setText).
     *
     * Instead of using a fixed delay, this method dynamically waits until no accessibility
     * events have been received for [quiescenceMs], then extracts the hierarchy.
     *
     * Flow emissions are suppressed during this operation to prevent race conditions
     * where debounced extractions could broadcast stale hierarchies.
     *
     * @param quiescenceMs How long to wait with no events before considering UI settled (default 50ms)
    ...
    fun extractAfterQuiescence(
        ...
        Log.d(TAG, "extractAfterQuiescence: suppressing flow emissions, waiting for first event then quiescence (${quiescenceMs}ms quiet)")
            ...
            kotlinx.coroutines.runBlocking {
                ...
                while (true) {
                    ...
                    if (elapsed >= maxWaitMs) {
                        Log.d(TAG, "extractAfterQuiescence: max wait time exceeded (${elapsed}ms), proceeding with extraction")
                        break
                    }

                    // Check if we've achieved quiescence
                    if (timeSinceLastEvent >= quiescenceMs) {
                        Log.d(TAG, "extractAfterQuiescence: quiescence achieved after ${elapsed}ms (no events for ${timeSinceLastEvent}ms)")
                        break
                    }
     ...
     *
     * IMPORTANT: The perf block MUST be closed BEFORE calling _hierarchyFlow.emit()
     * because emit() can suspend waiting for collectors. If a new accessibility event
     * triggers another extractAndCompare() while we're suspended on emit, its perf
     * block would become a child of this still-open block, causing nested accumulation.
     *
     * @param skipFlowEmit If true, skips emitting to the flow. Use this when the caller
     *                     will broadcast directly to avoid race conditions.
     */
    private suspend fun extractAndCompare(skipFlowEmit: Boolean = false) {
        ...
        try {
            ...
            if (structuralHash == lastStructuralHash) {
                ...
                resultToEmit = HierarchyResult.Unchanged(
                    hierarchy = hierarchy,
                    hash = structuralHash,
                ...
                Log.d(TAG, "Structure unchanged (hash=$structuralHash), entering animation mode for ${animationSkipWindowMs}ms")
                ...
                resultToEmit = HierarchyResult.Changed(
                    hierarchy = hierarchy,
                    hash = structuralHash,
                    extractionTimeMs = extractionTime
                )
