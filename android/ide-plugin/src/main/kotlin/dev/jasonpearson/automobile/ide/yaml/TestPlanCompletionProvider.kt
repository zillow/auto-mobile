...
class TestPlanCompletionProvider : CompletionProvider<CompletionParameters>() {
    ...
    private fun handleValueCompletion(
        ...
        if (stepMapping != null) {
            ...
            val enumValues = JsonSchemaIntrospector.collectEnumValues(schemaContext.schema, schemaContext.path)
    ...
    private fun handleKeyCompletion(
        ...
        val properties = when {
            ...
            path.firstOrNull()?.key == "params" -> {
                ...
                val toolSchema = toolName?.let { ToolDefinitionStore.getToolDefinitions()[it]?.inputSchema }
                toolSchema?.let {
                    JsonSchemaIntrospector.collectProperties(it, stripParamsPrefix(path))
                }.orEmpty()
            }
            else -> emptyMap()
        }
    ...
    private fun findStepMapping(element: PsiElement): YAMLMapping? {
        var current: PsiElement? = element
        while (current != null) {
            val sequenceItem = PsiTreeUtil.getParentOfType(current, YAMLSequenceItem::class.java) ?: return null
    ...
    private fun buildPathForKeyValue(
        ...
        val parentMapping = PsiTreeUtil.getParentOfType(keyValue, YAMLMapping::class.java) ?: return emptyList()
